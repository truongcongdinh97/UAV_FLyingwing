...
class AerodynamicsCalculator:
    ...
    def __init__(self):
        ...
        # Thông số động cơ (từ BOM: DXW D4250 600KV)
        self.motor_specs = {
            'model': 'DXW D4250 600KV',
            'kv': 600,  # RPM per volt
            'max_power': 650,  # watts (ước tính cho 6S)
            'quantity': 2,
            'voltage_6s': 22.2,  # V (6S nominal)
            'propeller_diameter': 0.330,  # m (13 inch - phù hợp 600KV)
            'propeller_pitch': 0.165,  # m (6.5 inch)
        }
        
        # Thông số pin (6S2P từ BOM)
        self.battery_specs = {
            'configuration': '4S2P',
    ...
    def _get_component_weights(self) -> Dict[str, float]:
        ...
        Ref: FlyingWing_BOM.csv
        """
        return {
            # I. Hệ thống Điều khiển & Dẫn đường
            'flight_controller': 0.025,  # LANRC F4 V3S Plus (có MPU6000 IMU tích hợp)
            'gps_module': 0.020,  # NEO-M8N GPS
            'compass': 0.005,  # QMC5883L
            'distance_sensor': 0.010,  # VL53L1X
            
            # II. Hệ thống Liên lạc
            'receiver': 0.005,  # Radiomaster XR1 Nano 2.4GHz
            
            # III. Hệ thống Hộp đen (ESP32 - có thể tháo rời, nguồn riêng)
            # Chỉ gắn khi bay test, không tính vào khối lượng bay thường
            'blackbox_esp32': 0.080,  # ESP32-CAM + GY-9250 + HC-SR04 + SD (tháo rời được)
            
            # IV. Máy tính đồng hành
            'raspberry_pi': 0.045,  # Raspberry Pi 3B+
            'camera_ov5647': 0.003,  # Pi Camera OV5647
            
            # V. Hệ thống động lực
            'motors': 0.380,  # 2x DXW D4250 600KV (~190g each)
            'esc': 0.200,  # 2x ESC 100A (~100g each)
            'servos': 0.220,  # 4x MG996R Split Elevon (~55g each)
            'propellers': 0.040,  # 2x props (3D printed)
            
            # VI. Hệ thống nguồn
            'battery': 1.200,  # 2x CNHL 6S 5200mAh 65C (6S2P config)
            'ubec': 0.020,  # Hobbywing 3A UBEC
            'buck_converters': 0.010,  # 2x Mini-360 3A
            
            # VII. Vật tư khác
            'wiring': 0.100,  # Dây điện, connectors
            'frame_3d_printed': 1.00,  # Khung máy bay in 3D
            'carbon_rods': 0.150,  # Thanh carbon phi 16mm
