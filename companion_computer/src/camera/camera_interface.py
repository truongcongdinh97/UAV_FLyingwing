...
class CameraInterface:
    ...
    def _start_picamera(self):
        """Khởi động Pi Camera với picamera2, cấu hình shutter speed và ISO"""
        ...
        # Tăng tốc độ màn trập và ISO nếu có cấu hình
        controls = {}
        shutter_us = self.config.get('shutter_speed_us', 1000) # microseconds
        iso = self.config.get('iso', 800)
        if shutter_us:
            controls['ExposureTime'] = max(1000, shutter_us) # min 1ms
        if iso:
            controls['AnalogueGain'] = iso / 100.0 # PiCamera2 dùng gain, ISO ~ gain*100
        if controls:
    ...
    def read_frame(self) -> Optional[Tuple[np.ndarray, float]]:
        """
        Đọc frame từ camera và trả về kèm timestamp
        Returns:
            (frame, timestamp) hoặc None nếu lỗi
        """
        ...
        try:
            ...
            if self.camera == "MOCK":
                ...
                import time as _t
                _t.sleep(1.0 / self.config.get('fps', 30))
