...
class RCModeController:
    ...
    def _is_critical_operation(self) -> bool:
        ...
        # Check if any critical operation is in progress
        for operation, is_active in self.critical_operations.items():
            if is_active:
                logger.debug(f"Critical operation in progress: {operation}")
                return True
        
        # Additional checks based on current mode
        if self.current_mode == AIMissionMode.SEARCH_RESCUE:
            # In search & rescue mode, always consider tracking as critical
            # This prevents mode switching when tracking potential rescue targets
            return True
    ...
    def set_critical_operation(self, operation: str, is_active: bool):
        """
        Set trạng thái của critical operation
        
        Args:
            operation: Tên operation ('emergency_tracking', 'alert_sending', etc.)
            is_active: True nếu operation đang active
        """
        if operation in self.critical_operations:
            self.critical_operations[operation] = is_active
            if is_active:
                logger.debug(f"Critical operation started: {operation}")
            else:
                logger.debug(f"Critical operation ended: {operation}")
        else:
            logger.warning(f"Unknown critical operation: {operation}")
    
    def get_critical_operations(self) -> Dict[str, bool]:
        """Lấy trạng thái của tất cả critical operations"""
        return self.critical_operations.copy()
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'critical_operations': self.get_critical_operations()
        }
