...
The companion computer runs **3 parallel threads**:
...
│   └─→ Queue → Thread 3
└─→ Thread 3: HTTP Upload to Ground Station
```

**Key Features**:
- **Non-blocking**: Camera never waits for AI or network
- **Fault-tolerant**: Thread crashes don't stop the system
- **Real-time**: Automatic frame dropping keeps system responsive
- **Monitored**: Watchdog timer auto-recovers from hangs
...
**Expected output**:
```
[INFO] Initializing companion computer...
...
[INFO] Detection: person (0.87) at (x=123, y=456)
[INFO] Target geolocation: lat=10.123456, lon=106.789012
[INFO] Target uploaded successfully
```
...
- **RAM**: Memory usage
- **Thread health**: Each thread reports status

```bash
# View live metrics
sudo journalctl -u uav-companion.service -f | grep "FPS\|RAM"

# Check thread status
sudo journalctl -u uav-companion.service -f | grep "thread"
```

### Watchdog Monitoring

The watchdog timer ensures system recovery:
- Default timeout: **60 seconds**
- Auto-resets system if any thread hangs
- Configurable in `config/system_config.yaml`

```bash
# Check watchdog status
sudo journalctl -u uav-companion.service | grep "Watchdog"

# Normal operation shows periodic kicks:
# [DEBUG] Watchdog kicked

# If timeout occurred:
# [CRITICAL] Watchdog timeout! Resetting system
```
...
sudo journalctl -u uav-companion.service | grep "Frame queue"

# Upload queue (AI → Network)
sudo journalctl -u uav-companion.service | grep "Upload queue"
```

**Expected behavior**:
- "Frame queue full, dropping old frame" = **Normal** (real-time design)
- "Upload queue full" = **Warning** (network too slow)
