...
- **Baudrate**: 921600 bps (1.5Mbaud recommended for high throughput)
- **Protocol**: MAVLink v2.0 (primary) hoặc MSP (backup)
- **Note**: Dây nối UART giữa Pi và FC phải ngắn (< 15cm) và bọc nhiễu tốt
...
### 2.2. AI Detection & Target Geolocation
...
**HTTP REST API**: Target Geolocation Upload
- **Port**: 5000 (Flask default)
- **Protocol**: HTTP POST
- **Endpoint**: `/api/target`
- **Pattern**: Fire-and-forget (queued, non-blocking)
...
**Channel 4: Target Geolocation** (Medium Priority)
- HTTP POST `/api/target`
- Fire-and-forget pattern
- Queued upload (maxsize=50)
- Format: JSON

**Channel 5: Data Logging** (Low Priority)
- Log file transfer
- Mission data download

---

## 3.4. HTTP API: Target Geolocation Upload

### Endpoint: POST /api/target

**Purpose**: Upload detected target GPS coordinates from companion computer to ground station

**Architecture**: Fire-and-forget pattern
- Detection → Queue (maxsize=50)
- Background worker thread uploads
- Non-blocking (never delays detection pipeline)
- Automatic retry on failure

**Request Format**:
```json
{
  "timestamp": "2025-01-20T14:32:45.123Z",
  "class_name": "person",
  "confidence": 0.87,
  "bbox": {
    "x1": 123,
...
{
  "status": "success",
  "message": "Target received",
  "target_id": "tgt_20250120_143245_001"
}
```

**Error Handling**:
- Network failure: Queued for retry
- Server down: Logged locally to `target.jsonl`
- Queue full: Oldest targets dropped (FIFO)
...
**Dashboard Behavior**:
- Receives target via WebSocket
- Immediately displays on map (icon + marker)
- Updates target list panel
